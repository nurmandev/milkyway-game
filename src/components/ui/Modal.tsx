...
const Modal = ({
  ...
  if (!isOpen || !isOnClient) return null;

  return ReactDOM.createPortal(
    <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-[#00000096] z-[999] touch-manipulation overscroll-none">
      <div className="w-[65%] h-auto relative animate-popup">
        ...
        <div className="w-full absolute top-0 left-0 h-full flex flex-col items-center justify-center">
          ...
          <div className="w-[98.5%] h-[15%] mt-1 absolute top-0 left-0 flex items-center justify-center">
            <h1 className="text-white modal-title text-[2.5vw]">{modalType}</h1>
          </div>
          <div className="h-full w-[98.5%]">{children}</div>
        </div>

        <button
          className="absolute right-[-5%] top-[-2%] h-[12%] w-[10%] min-w-[44px] min-h-[44px]"
          onClick={handleClick}
